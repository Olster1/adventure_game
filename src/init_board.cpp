...
void initPlayerBoard(GameState *gameState) {
    ...
    gameState->averageStartBoardCount = 0;
    ...
    placeGoblinTower(gameState, p);
    placeGoblinHouse(gameState, p);
    placeTower(gameState, p);
    ...
    //NOTE: Calculate the average position now
    float dividend = 1.0f / (float)gameState->averageStartBoardCount;
    gameState->cameraPos.x = gameState->averageStartBoardPos.x * dividend;
    gameState->cameraPos.y = gameState->averageStartBoardPos.y * dividend;
