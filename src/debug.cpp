...
void drawDebugAndEditorText(GameState *gameState, Renderer *renderer, float fauxDimensionX, float fauxDimensionY, float windowWidth, float windowHeight, float dt, float16 fovMatrix) {
	...
	if(global_platformInput.keyStates[PLATFORM_KEY_1].pressedCount > 0 && global_platformInput.keyStates[PLATFORM_KEY_COMMAND].isDown) {
		int value = gameState->debug_commandBuffer.debug_InputBufferOpenState;
		if(value == 0) {
			wasJustOpened = true;
		}
		value++;
		if(value > DEBUG_COMMAND_BUFFER_OPEN_FULL) {
			value = 0;
		}

		gameState->debug_commandBuffer.debug_InputBufferOpenState = (DebugCommandBufferState)value;
	}

	if(!wasJustOpened && gameState->debug_commandBuffer.debug_InputBufferOpenState != DEBUG_COMMAND_BUFFER_CLOSED) {
		debug_drawUpdateInputBuffer(renderer, gameState, fauxDimensionX, fauxDimensionY);
	}
}